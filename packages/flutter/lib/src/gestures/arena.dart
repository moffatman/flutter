...
class GestureArenaEntry {
  ...
  void resolve(GestureDisposition disposition, {double? priority}) {
    _arena._resolve(_pointer, _member, disposition, priority: priority);
  }
}
...
class _GestureArena {
  ...
  final Map<GestureArenaMember, double> priorities = <GestureArenaMember, double>{};
...
class GestureArenaManager {
  ...
  void gavel(int pointer) {
    ...
    if (!state.isOpen && state.priorities.isNotEmpty) {
      _tryToResolveArena(pointer, state);
    }
  }
  ...
  void _resolve(int pointer, GestureArenaMember member, GestureDisposition disposition, {double? priority}) {
    assert(priority == null || priority >= 0);
    final _GestureArena? state = _arenas[pointer];
    if (state == null) {
    ...
    if (disposition == GestureDisposition.rejected) {
      ...
      if (priority != null) {
        state.priorities[member] = priority;
      }
  ...
  void _tryToResolveArena(int pointer, _GestureArena state) {
    ...
    if (state.members.length == 1) {
      ...
      double bestPriority = double.infinity;
      GestureArenaMember? winner;
      for (final MapEntry<GestureArenaMember, double> priority in state.priorities.entries) {
        // Need to use < so that tie is broken by later map insertion (deeper inside element tree)
        if (priority.value < bestPriority) {
